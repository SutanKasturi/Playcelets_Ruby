module PlayInvitationsHelper
  def select_play_invitation_duration_tag(form, object, options={})
    ...
    options.merge(onchange: 'window.PlayInvitation.displayEndTime(this)', include_blank: '-- Select End Time --')
  end

  def select_play_invitation_family_tag(form, object, options={})
    values = @families.map{|family| [family.name, family.id]}
    values.unshift(['-- Select Family --', ''])
    select_tag :family_id,
    options_for_select(values, selected: (@family_id || object.family_id)),
    options.merge(onchange: 'window.PlayInvitation.showChildren(this)', include_blank: false)
  end

  def select_play_invitation_parent_tag(parents, options={})
    select :play_invitation, options[:name] || :parent_id, 
    options_for_select(parents.map{|parent| [parent.name, parent.id]}), 
    options.merge(include_blank: '-- Select Sender Parent --')
  end

  def select_play_invitation_child_tag(children, options={})
    select :play_invitation, options[:name] || :child_id, 
    colored_children_options_for_select(children), 
    options.merge(include_blank: '-- Select Child --')
  end
